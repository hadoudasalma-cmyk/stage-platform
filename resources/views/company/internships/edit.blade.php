...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-800">Modifier l'offre</h1>
</div>
...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <form method="POST" action="{{ route('company.internships.update', $internship) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="field" value="{{ old('field', $internship->field) }}" required
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Ville <span class="text-red-500">*</span></label>
                    <input type="text" name="city" value="{{ old('city', $internship->city) }}" required
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Durée (mois) <span class="text-red-500">*</span></label>
                    <input type="number" name="duration" value="{{ old('duration', $internship->duration) }}" required min="1" max="24"
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <option value="active"  {{ old('status', $internship->status) === 'active'  ? 'selected' : '' }}>Active</option>
                        <option value="closed"  {{ old('status', $internship->status) === 'closed'  ? 'selected' : '' }}>Fermée</option>
                        <option value="expired" {{ old('status', $internship->status) === 'expired' ? 'selected' : '' }}>Expirée</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4" required
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none">{{ old('description', $internship->description) }}</textarea>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Profil recherché</label>
                    <textarea name="requirements" rows="3"
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none">{{ old('requirements', $internship->requirements) }}</textarea>
                </div>

                <!-- Rémunération -->
                <div class="md:col-span-2" x-data="{ paid: {{ $internship->is_paid ? 'true' : 'false' }} }">
                    <label class="flex items-center gap-2 cursor-pointer mb-3">
                        <input type="checkbox" name="is_paid" value="1" x-model="paid"
                            class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                            {{ $internship->is_paid ? 'checked' : '' }}>
                    ...
                    <div x-show="paid" x-transition>
                        ...
                        <input type="number" name="salary" value="{{ old('salary', $internship->salary) }}" min="0"
                            class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('company.internships.show', $internship) }}"
                    class="px-6 py-2.5 border border-gray-300 text-gray-600 rounded-lg text-sm hover:bg-gray-50 transition">
                    Annuler
                </a>
            </div>
        </form>
